...
knl = lp.split_iname(knl, "i", BLOCKSIZEy, outer_tag="g.0", inner_tag="l.1")
knl = lp.split_iname(knl, "j", BLOCKSIZEx, outer_tag="g.1", inner_tag="l.0")
knl = lp.split_iname(knl, "k", BLOCKSIZEx)
...
tconfig = ThreadConfig(BLOCKSIZEx*BLOCKSIZEy, n/BLOCKSIZEx*n/BLOCKSIZEy)

model = PerfModel(gstats, kstats, tconfig, np.dtype(np.float32), active_blocks=1)
cycles = model.compute_total_cycles()
#print "actual runtime: ", (evt.profile.END - evt.profile.START)*1e-9
print "total predicted time (ms): ", cycles/(gstats.sm_clock_freq*10**6)
